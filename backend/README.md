...
- `backend/api/weather.py`: still a stub. This is the teammate's connector
  to build, following the same pattern as `traffic_air.py`/`traffic_sea.py`.
- `backend/ai_agents/*`: local LLM impact simulation, rerouting, and
  autonomous client communication (Steps 4 and 5 of the workflow). This is
  the AI track's responsibility.
